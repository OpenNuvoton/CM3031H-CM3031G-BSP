...
/**
  ...
  *                         - \ref TIMER_INTER_CAPTURE_OFF
  *                         - \ref TIMER_INTER_CAPTURE_SOURCE_ACMP0
  *                         - \ref TIMER_INTER_CAPTURE_SOURCE_ACMP1
  *                         - \ref TIMER_INTER_CAPTURE_SOURCE_HXT
  *                         - \ref TIMER_INTER_CAPTURE_SOURCE_LXT
  *                         - \ref TIMER_INTER_CAPTURE_SOURCE_HIRC
  *                         - \ref TIMER_INTER_CAPTURE_SOURCE_LIRC
  *
  * @return     None
  *
...
{
    if(u32Src == TIMER_INTER_CAPTURE_OFF)
    {
        timer->CTL = (timer->CTL & ~(TIMER_CTL_CAPSRC_Msk)) |
                     (TIMER_CAPTURE_SOURCE_FROM_PIN);
    }
    else
    {
        timer->CTL = (timer->CTL & ~(TIMER_CTL_CAPSRC_Msk)) |
                     (TIMER_CAPTURE_SOURCE_FROM_INTERNAL);
...
/**
  ...
  * @param[in]  u32DropCount    This parameter has no effect in CM3031 series BSP
  * @param[in]  u32Timeout      This parameter has no effect in CM3031 series BSP
...
/**
  ...
  * @param[in]  timer   The base address of Timer module
  * @param[in]  u32Mask The mask of modules (EPWM/BPWM, EADC, and PDMA) trigger by timer. Is the combination of
